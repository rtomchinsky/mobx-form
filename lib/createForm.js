...
function createForm(fields) {
    ...
    var form = {
        ...
        each: function (cb) {
            lodash_1.forEach(formFields, cb);
        },
        reset: function () { return form.each(mobx_1.action(function (v) { return v.reset(); })); },
        commit: function () { return form.each(mobx_1.action(function (v) { return v.commit(); })); },
        submit: function (onSubmit) {
            return __awaiter(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            lodash_1.forEach(formFields, mobx_1.action(function (it) {
                                it.isTouched = true;
                            }));
    ...
    var formFields = mobx_1.observable(lodash_1.mapValues(fields, function (options) {
        return new FormValue_1.FormValue(form, options);
    }));
    form.fields = formFields;
    lodash_1.forEach(formFields, function (field) { return field.initialize(); });
    mobx_1.extendObservable(form, {
    ...
    mobx_1.reaction(function () { return lodash_1.map(formFields, function (it) {
        ...
        mobx_1.runInAction(function () {
            formMetadata.isPristine = !lodash_1.some(fields, function (f) { return !f.isPristine; });
        });
        validateForm();
    }, {
    ...
    validation.subscription = sub.asObservable()
        ...
        .switchMap(function () {
        return Promise.all(lodash_1.map(formFields, function (it) { return it.validate(); }))
